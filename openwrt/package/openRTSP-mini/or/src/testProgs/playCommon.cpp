...
//       << " [-w <width> -h <height>] [-f <frames-per-second>] [-y] [-H] [-Q [<measurement-interval>]] [-F <filename-prefix>] [-b <file-sink-buffer-size>] [-B <input-socket-buffer-size>] [-I <input-interface-ip-address>] [-m] [<url>|-R [<port-num>]] (or " << progName << " -o [-V] <url>)\n";
       << " [-w <width> -h <height>] [-f <frames-per-second>] [-y] [-H] ] [-F <filename-prefix>] [-b <file-sink-buffer-size>] [-B <input-socket-buffer-size>] [-I <input-interface-ip-address>] [-m] [<url>|-R [<port-num>]] (or " << progName << " -o [-V] <url>)\n";
  shutdown();
}
...
//!not_print_SDP  *env << "Opened URL \"" << streamURL << "\", returning a SDP description:\n" << sdpDescription << "\n";
...
//!	       outputAVIFile ? "avi" : generateMP4Format ? "mp4" : "mov");
	       generateMP4Format ? "mp4" : "mov");
    }

//sprintf(outFileName, "test.mp4");//!  
...
static void periodicQOSMeasurement(void*) {//void* - clientData
  struct timeval timeNow;
  gettimeofday(&timeNow, NULL);
